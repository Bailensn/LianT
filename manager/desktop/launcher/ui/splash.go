...
var (
	bgDark     = color.NRGBA{R: 0x16, G: 0x1B, B: 0x26, A: 0xFF}
	accent     = color.NRGBA{R: 0x4F, G: 0x8B, B: 0xFF, A: 0xFF}
...
func (s *Splash) Run() {
	...
	for {
		...
		case app.FrameEvent:
			...
			s.layout(gtx)
			if !s.centered.Swap(true) {
				s.win.Perform(system.ActionCenter)
			}
